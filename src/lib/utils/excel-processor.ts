...
import { condicionIva, categorias, marcas } from "./index";
...
function validateWorksheets(workbookData: Record<string, (string | number)[][]>): string[] {
  const requiredSheets = ["DISTRIBUIDOR", "LISTA DE PRECIOS", "LISTA DE PRECIOS TRADICIONAL", "CLIENTES"];
 ...
 */
function validateAndAddMissingColumns(workbookData: Record<string, (string | number)[][]>): {
  ...
  for (const [sheetName, requiredColumns] of Object.entries(REQUIRED_COLUMNS)) {
    ...
    for (const requiredCol of requiredColumns) {
      ...
      const found = existingColumnsLower.some(
        (existing) =>
          existing.includes(requiredColLower.split(" ")[0]) || requiredColLower.includes(existing.split(" ")[0])
      );
    ...
    if (missingColumns.length > 0) {
      ...
      for (const missingCol of missingColumns) {
        ...
        if (sheetName === "DISTRIBUIDOR" && missingCol === "CUIT") {
          ...
          const visitaLunesIndex = newHeaders.findIndex(
            (h) => h.toLowerCase().includes("visita") && h.toLowerCase().includes("lunes")
          );
...
function processDistribuidorWorksheet(errors: string[], data: (string | number)[][]): ProcessedDistribuidor[] {
  ...
  const condicionIvaIndex = headers.findIndex(
    (h) => h.toLowerCase().includes("condicion") && h.toLowerCase().includes("iva")
  );
...
function processListaPreciosTradicionalWorksheet(errors: string[], data: (string | number)[][]): ProcessedListaPreciosTradicional[] {
  ...
  const headers = data[0].map((h) => String(h));
  const codigoProductoIndex = headers.findIndex((h) => h.toLowerCase() === "código" || h.toLowerCase() === "codigo");
  const marcaIndex = headers.findIndex((h) => h.toLowerCase() === "marca");
  const categoriaIndex = headers.findIndex((h) => h.toLowerCase() === "categoria");
  const precioSinIvaIndex = headers.findIndex((h) => h.toLowerCase() === "precio sin iva");
  const ivaIndex = headers.findIndex((h) => h.toLowerCase() === "% iva" || h.toLowerCase() === "iva");
  const precioConIvaIndex = headers.findIndex((h) => h.toLowerCase() === "precio con iva");
  ...
  return data.slice(1).map((row) => {
    ...
    headers.forEach((header, index) => {
      ...
      if (index === precioSinIvaIndex || index === precioConIvaIndex) {
        value = parseNumericValue(value);
      } else if (index === ivaIndex) {
        // Always set IVA to 21%
        value = 21;
      }
    ...
    // Extract Marca and Categoria from Codigo (Rule 2: Extract brands and categories by product code)
    if (codigoProductoIndex >= 0) {
      const codigo = row[codigoProductoIndex];
      const { marca, categoria } = extractMarcaAndCategoriaFromCodigo(Number(codigo));
      result[headers[marcaIndex]] = marca;
      result[headers[categoriaIndex]] = categoria;
    }

    // Rule 3: Validate prices with/without IVA (Always use 21% IVA rate)
    ...
    // Formula: priceWithoutIVA = priceWithIVA / 1.21
      ...
      precioConIva > 0 &&
      (!precioSinIva || precioSinIva === "SN" || precioSinIva === 0)
    ) {
...
function processClientesWorksheet(
  ...
  if (codigoListaPreciosIndex === -1) {
    const visitaLunesIndex = headers.findIndex(
      (h) => h.toLowerCase().includes("visita") && h.toLowerCase().includes("lunes")
    );
  ...
  const codigoIndex = headers.findIndex(
    (h) => h.toLowerCase().includes("codigo") && !h.toLowerCase().includes("lista")
  );
...
/**
 * Extract Marca and Categoria from product code
 * Rules: Analyze product code patterns to determine brand and category
 */
function extractMarcaAndCategoriaFromCodigo(codigo: number): { marca: string; categoria: string } {
  // Default values
  const marca =
    marcas.find((m) => {
      if (m.codigo === codigo) {
